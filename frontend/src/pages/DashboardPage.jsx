...
function StatCard({ icon: Icon, label, value, color, delay }) {
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
      className="neo-box p-4 sm:p-5 flex items-center sm:items-start gap-3 sm:gap-4 bg-white relative min-w-0">
      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl ${color} border-2 border-black flex items-center justify-center flex-shrink-0 shadow-[2px_2px_0px_#000]`}>
        <Icon className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-[10px] sm:text-xs font-black uppercase text-slate-500 font-display truncate">{label}</p>
        <p className="text-2xl sm:text-3xl font-black text-slate-900 font-display mt-0.5 truncate">{value}</p>
      </div>
    </motion.div>
  );
}
    ...
    <DashboardLayout>
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 pb-12 w-full min-w-0">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 min-w-0 w-full max-w-full">
          <div className="min-w-0">
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-slate-900 font-display tracking-tight break-words">
              Welcome back, <span className="neo-tag-pink px-2 py-0.5 inline-block my-0.5">{user?.name?.split(' ')[0]}</span>
            </h1>
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
            <Link to="/live" className="neo-btn-white text-xs py-2 sm:py-2.5 px-3 sm:px-4 flex-1 sm:flex-none justify-center">
              <Zap className="w-4 h-4 text-[#EC4899]" /> Live Mode
            </Link>
            <Link to="/create-quiz" className="neo-btn-pink text-xs py-2 sm:py-2.5 px-3.5 sm:px-5 flex-1 sm:flex-none justify-center">
        ...
        {activityData.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-6 min-w-0 w-full max-w-full">
            {/* Quiz Activity Chart */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
              className="neo-box p-4 sm:p-6 lg:col-span-3 bg-white min-w-0 w-full max-w-full overflow-hidden">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="font-extrabold text-base sm:text-lg text-slate-900 font-display">Quiz Activity</h2>
                  <p className="text-xs font-bold text-slate-500">Quizzes taken over the last 7 days</p>
                </div>
                <BarChart2 className="w-5 h-5 sm:w-6 sm:h-6 text-[#EC4899]" />
              </div>
              <div className="w-full h-[200px] sm:h-[220px] min-w-0 overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
              className="neo-box p-4 sm:p-6 lg:col-span-2 bg-white min-w-0 w-full max-w-full overflow-hidden">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="font-extrabold text-base sm:text-lg text-slate-900 font-display">Avg. Scores</h2>
          ...
          !loading && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
              className="neo-box p-4 sm:p-5 bg-white flex items-center gap-3 sm:gap-4 text-slate-500 min-w-0">
        ...
        {/* My Recent Quizzes */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="min-w-0 w-full max-w-full">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-black text-lg sm:text-xl text-slate-900 font-display">My Recent Quizzes</h2>
          ...
          {myQuizzes.length === 0 ? (
            <div className="neo-box p-6 sm:p-12 text-center bg-white relative">
              <RotatingBadgeDoodle text="Create First Quiz • " icon={BookOpen} className="mx-auto mb-4" />
              <h3 className="font-black text-xl sm:text-2xl font-display text-slate-900 mb-2">No quizzes created yet</h3>
              <p className="text-slate-600 font-medium text-xs sm:text-sm mb-6 max-w-md mx-auto">
                Generate your first quiz with AI or craft custom questions to engage your campus peers.
              </p>
              <Link to="/create-quiz" className="neo-btn-pink py-2.5 sm:py-3 px-6 sm:px-8 text-xs sm:text-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 w-full max-w-full min-w-0">
              {myQuizzes.map((quiz) => (
                <div key={quiz.id} className="neo-box p-4 sm:p-5 bg-white flex flex-col justify-between min-w-0">
                  <div>
        ...
        {/* Quick Actions Grid */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="min-w-0 w-full max-w-full">
          <h2 className="font-black text-lg sm:text-xl text-slate-900 font-display mb-4">Quick Actions</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 w-full max-w-full min-w-0">
            {[
              ...
              <Link key={i} to={action.to}
                className="neo-box p-4 sm:p-5 bg-white flex flex-col items-center justify-center text-center gap-2 group hover:translate-y-[-3px] min-w-0">
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl ${action.bg} border-2 border-black flex items-center justify-center shadow-[2px_2px_0px_#000]`}>
                  <action.icon className="w-5 h-5 sm:w-6 sm:h-6" />
                </div>
                <span className="font-extrabold text-xs sm:text-sm text-slate-900 font-display line-clamp-1">{action.label}</span>
              </Link>
            ))}
          </div>
        </motion.div>
